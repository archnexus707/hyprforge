...
if git clone --recursive -b $tag https://github.com/hyprwm/hyprlang.git "$SRC_DIR"; then
	...
	if ! cmake --build "$BUILD_DIR" --config Release --target hyprlang -j "$(nproc 2>/dev/null || getconf _NPROCESSORS_CONF)"; then
		echo -e "${ERROR} hyprlang cmake --build failed; refusing to install a stale/partial build" 2>&1 | tee -a "$MLOG"
		exit 1
	fi
